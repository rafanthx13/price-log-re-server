module.exports = (app) => {
		...
		const getShopByCity = (city) => {
			if(notExists(city))
				throw new ValidateError('City is invalid data')
		...
		const save = async (shop) => {
			...
			if(exist(shopInBD))
				throw new DuplicateError(`Shop '${shop.name}' already exists in the database.`);

			return app.db('shop').insert(shop).then( id => {
		...
		return { getAll, getOnlyShop, getShopByCity,
		 save, update, remove };
}